...
浏览器打开 `http://localhost:8080/mock-admin.html`，共 **7 个标签页**，涵盖端点查看、在线测试、统计分析、配置编辑、录制回放等全部操作——新用户无需任何命令行工具即可完成全部适配工作。

---

### 标签 1：端点列表

展示所有已注册的 HTTP 和 WebSocket 端点，支持按 id / path / 描述**实时搜索**过滤，顶部统计卡片显示总端点数、HTTP / WS / 脚本端点数量。
...
**无需 Postman / curl**，直接在页面发起请求并查看响应。

使用步骤：
1. 从下拉框选择要测试的端点（自动显示方法、路径、必填参数）
2. 在自动生成的参数表单中填写测试值
3. 勾选「**同时开启录制**」可在发送请求的同时自动记录响应（便于后续回放）
4. 点击「**发送请求**」，响应状态码、耗时、响应体即时显示

> 此功能内部调用本地 Mock 服务，所有请求都会触发完整的 Mock 逻辑（校验 → 响应体解析 → 模板替换 → 延迟模拟）。

---

### 标签 3：统计信息

实时查看服务运行数据，无需 Prometheus / Grafana。
...
| 端点 ID | 端点唯一标识 |
| 总请求数 | 生命周期内累计命中次数 |
| 成功（<400） | HTTP 状态码 < 400 的请求数及占比进度条 |
| 错误（≥400） | HTTP 状态码 ≥ 400 的请求数及占比 |

**最近请求日志**（最多保留 200 条，最新在前）

每行显示：方法、路径、端点 ID、状态码、耗时。点击行可展开查看请求参数和响应体预览（前 200 字符）。
